...
arch_voi  <- voi_est(arch_data, combn(c("Tres","t_tit","trate","sdb","sdl","Hmax","t_vax","arate","puptake"), 2, simplify = FALSE)) %>%
...
arch_lab  <- arch_fit %>% group_by(disease, location) %>% 
             ...
             mutate(across(c(xlabel, ylabel), ~ case_when(. == "Tres" ~ "Response Time (day)",
...
# arch_fit <- arch_fit %>% group_by(location, disease, strategy) %>%
#             mutate(SECpc = (SECpc - min(SECpc)) /(max(SECpc) - min(SECpc))) %>%
#             ungroup()

gg <- ggplot(arch_fit, aes(x = xaxis, y = yaxis, z = SECpc, color = SECpc)) +
