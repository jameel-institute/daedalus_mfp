function f=p2Plot(data,tvec,f1,x1,p2)

numSectors=length(data.obj);
...
h1=f1(ind,4);
d1=f1(ind,5);
v1=f1(ind,6);%heSimCovid19 output
v2=f1(ind,7);%heSimCovid19 output
v3=f1(ind,8);%heSimCovid19 output
v4=f1(ind,9);%heSimCovid19 output
sn=f1(ind,14);%heSimCovid19 output
if dodiff==1
...
maxY=max([100000,(5/4)*d1'/scal4,(5/4)*thresh/scal4,(5/4)*h1'/scal4]);%inc2;h2
...
%hh1=plot(t1,s1/scal1,'-','linewidth',lw,'color','blue');
%hh0=plot(t1,100000*sn./s1,'-','linewidth',lw,'color','cyan');
...
legend([hh3,hh4,hh5],'Hospital Occupancy (per 100m)','Deaths (per 100m)','Vaccinated (per 100k)','location','northeast');%'Position',[-0.29 0.27 1 1]);
