...
if strcmp(inp3,'No Closures');
    data.xconf = [ones(2,lx);NaN(2,lx);ones(1,lx)];
    data.hw    = [zeros(2,lx);NaN(2,lx);zeros(1,lx)];
    ...
    data.inext = [2,5,6];%index corresponds to event number in event function, value corresponds to number of next configuration
elseif strcmp(inp3,'School Closures');
    data.xconf = [ones(2,lx);data.x_schc(:,2)';data.x_schc(:,1)';ones(1,lx)];
    data.hw    = [zeros(2,lx);data.wfh;data.wfh;zeros(1,lx)];
    data.imand = [3];
    data.ev_fn = @dd_set_rules_seclosures;
    data.inext = [2,3,4,5,6];
