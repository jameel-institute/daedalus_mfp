function poptim=fitEpi20(ydata)
%%

load('UK36.mat','data');
load('xmins.mat','xmins');
...
for i=1:length(repCol)
    X(:,repColSum(i)+1:repColSum(i+1))=repmat(xmins(:,i),1,repCol(i));
end
...
xdata=(80:305);%20th March-31st August incl.
ydata=ydata(1:size(xdata,2));

ub=[0,      100,    2.75,   1.00*ones(1,3)];
x0=[-50,    90,     2.75,   0.50*ones(1,3)];
lb=[-100,   60,     1.01,   0.00*ones(1,3)];%t0, tld, R0, deltas
...
function f=sim2fit(params,data,xdata,Xfit)
    ...
    tvec=[params(1),params(2),122,153,183,214,245,275,306];
    
    [pr,vx,NN,n,ntot,na,NNbar,NNrep,Dout,beta]=hePrepCovid19(data,numInt,params(3),[params(4)*ones(1,2),params(5)*ones(1,3),params(6)*ones(1,2)]);
    pr.sw=0;
    data.wfhAv=[zeros(1,length(data.G));repmat(data.wfhAv(2,:),3,1);repmat(data.wfhAv(1,:),4,1)];
    
    [simu,~,~]=heRunCovid19(pr,vx,n,ntot,na,NN,NNbar,NNrep,Dout,beta,Xfit,tvec,0,data);
