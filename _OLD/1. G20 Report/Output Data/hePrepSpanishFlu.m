function [pr,vx,NN,n,ntot,na,NNbar,NNrep,Dout,beta]=hePrepSpanishFlu(data,numInt,inp1,inp2)%,R0,del)
...
%Adherence to NPIs
pmod=0.5400;%lockdown delta%from fitting
...
pr.p3=0.0068;%proportion of asymptomatic self-isolating
pr.p4=0.0068;%proportion of symptomatic self-isolating
...
pr.Hmax=32.4021*sum(data.Npop)/(10^5);
%pr.thu=0.90*pr.Hmax;
pr.thl=0.30*pr.Hmax;
...
% pr.Tm=pr.Tm+10;
% pr.p3=0;pr.p4=0;
% pr.betamod=[1,0.60*ones(1,numInt)];
% pr.Hmax=12*sum(data.Npop)/(10^5);
...
pr.Tm=pr.Tm-30;
pr.p3=0.025;pr.p4=0.025;
pr.betamod=[1,0.40*ones(1,numInt)];
pr.Hmax=252*sum(data.Npop)/(10^5);
...
%arate=650.1573*sum(data.Npop)/(10^5);
...
%Period 1 - working-age
vx.startp1= tpoints(1);
vx.aratep1= [0;0;arate;0];%to be split across all economic sectors in heSimCovid19vax.m

%Period 2 - retired-age
vx.startp2= tpoints(2);
vx.aratep2= [0;0;0;arate];
...
% ps=     0.6;
ihr=    ps*[0.06768	0.01180	0.01417	0.02912	0.05037	0.08657	0.07319	0.06532	0.04880	0.03542	0.04880	0.03463	0.09129	0.12278	0.14639	0.14639	0.14639];
phgs=   ihr./ps;
...
ifr=    ps*[0.02284	0.00398	0.00478	0.00983	0.01700	0.02922	0.02470	0.02205	0.01647	0.01195	0.01647	0.01169	0.03081	0.04144	0.04941	0.04941	0.04941];
pdgh=   ifr./ihr;
