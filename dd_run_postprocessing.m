...
Sclass   = yout(:, 0*ln+1: 1*ln) + yout(:, 8*ln+1: 9*ln) + yout(:, 9*ln+1:10*ln); 
...
output.Sout       = [output.Sout;Sclass(2:end,:)];
...
if inext==6;
    ...
    output.f     = [output.Tout,output.Xout,output.hwout,...
                    ...
                    output.Sout,output.Hadout,p2.Tres*ones(size(output.Tout))];
