function p2SimRand_beta(igroups,diseases)%,strategies)
    ...
    for h = 1:ligroups;
        ...
        inputs(i,:) = [i,NaN,NaN,ldata.Npop',ldata.NNs(1:45)',...
                       ...
                       NaN,ldata.la];
        [ldata,~,~] = dd_set_disease(ldata,'Covid Wildtype');%to define wnorm and Td_CWT
        %for l = 1:2;if l==2;ldata.t_vax = min(ldata.t_vax,100);end
                   
        for j = 1:ldis;
        
        inp2           = diseases{j};    
        [ldata,dis,p2] = dd_set_disease(ldata,inp2);
...
%         %plots = p2Plot(data,f,p2,g,cost,ccost_t,sec(1),inp1,inp2,inp3);
        ...
        for j = 1:ldis;
        for k = 1:lstrat;
        dd_store_output([inputs,squeeze(outputs(j,k,:,:))],strcat(string(igroups{h}),'_',string(diseases{j})),taskdir);
        end
        end
