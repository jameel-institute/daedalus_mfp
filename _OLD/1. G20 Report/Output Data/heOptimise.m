...
% pr.betamod(2:end)=min(1.30*pr.betamod(2:end),1);
...
lb=0.8*xmin;
if numSect==35
    lb(33:numSect:end)=xmin(33);
    if scenB==1
        lb(32:numSect:end)=0.8;
    end
elseif numSect==36
...
function [c,cex]=epiConstraint(pr,vx,n,ntot,na,NN,NNbar,NNrep,Din,beta,Xit,tvec,hospThresh,data)
    ...
    [~,h,~]=heRunCovid19(pr,vx,n,ntot,na,NN,NNbar,NNrep,Din,beta,Xit,tvec,0,data);
    c=h-hospThresh;
    cex=[];
