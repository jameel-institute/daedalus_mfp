function p2SimRand
    ...
    vcrit      = 1-(1./[1.58,2.5,5.9436,2.87,1.75]);
    ...
    strategies = {'No Closures','Economic Closures','No Closures','No Closures','Economic Closures';
                  'No Closures','Elimination','No Closures','No Closures','Elimination';
                  'No Closures','Elimination','No Closures','Elimination','Elimination'};
    lstrat     = 1;%length(strategies);
    inputs     = zeros(nsamples,493);
    output     = zeros(ldis,lstrat,nsamples,2,4);
    vc         = readtable('40vax_pc_gdp.csv');
    intcost    = zeros(nsamples,ldis);
...
%     intcost(i)= v100*(max(ldata.puptake,0.70)-ldata.puptake); 
    ...
    if l==1;
        ...
        ldata.puptake = 0.5*vcrit(j);
        %ldata.puptake = max(ldata.puptake,0.70);
    elseif l==2;
        ldata.puptake = vcrit(j);
    end
    ...
    for j = 1:ldis;
    for k = 1:lstrat;
    T                          = array2table([inputs,squeeze(output(j,k,:,1,:)),squeeze(output(j,k,:,2,:)),squeeze(intcost(:,j))]);
