function [data,f,g]=p2Run_alt(data,dis,inp3,int,Xit,p2);
...
if t<p2.t_tit;   
    T = 0;
elseif t<p2.end && i~=5;
    T = p2.trate*sum(data.Npop)/10^5;%number of tests per day
else       
    T = 0;    
end
...
Pct     = 0.34;
Cct     = Tct*Pct;
Cct_a   = Cct*(1-dis.ps)*E  ./sum(E+Ev1);
Cct_s   = Cct*(dis.ps)  *E  ./sum(E+Ev1);
Cct_av  = Cct*(1-dis.ps)*Ev1./sum(E+Ev1);
Cct_sv  = Cct*(dis.ps)  *Ev1./sum(E+Ev1);

Cmax    = Cct;
Qr      = min(max(0,-0.8),Ts)./Ts;
...
I       = (red*Ina+Ins)+Qr.*(red.*Isa+Iss)+(1-trv1)*(red.*Inav1+Insv1)+Qr.*(1-trv1).*(red.*Isav1+Issv1);
...
Inadot=     sig1.*E     - min(sig1.*E,  Cct_a)    -g1.*Ina;               
Insdot=     sig2.*E     - min(sig2.*E,  Cct_s)    -(g2+h).*Ins;            
Inav1dot=   sig1.*Ev1   - min(sig1.*Ev1,Cct_av)    -g1.*Inav1;              
Insv1dot=   sig2.*Ev1   - min(sig2.*Ev1,Cct_sv)    -(g2_v1+h_v1).*Insv1;    

Isadot=     min(sig1.*E,  Cct_a)    -g1.*Isa;
Issdot=     min(sig2.*E,  Cct_s)    -(g2+h).*Iss;
Isav1dot=   min(sig1.*Ev1,Cct_av)   -g1.*Isav1;
Issv1dot=   min(sig2.*Ev1,Cct_sv)   -(g2_v1+h_v1).*Issv1;

Hdot=       h.*Ins      +h.*Iss        -(g3+mu).*H;
Hv1dot=     h_v1.*Insv1 +h_v1.*Issv1   -(g3+mu).*Hv1;

Rdot=       g1.*Ina     +g1.*Isa   +g2.*Ins        +g2.*Iss       +g3.*H      -nu.*R      -v1rater;
Rv1dot=     g1.*Inav1   +g1*Isav1  +g2_v1.*Insv1   +g2_v1.*Issv1  +g3.*Hv1    +v1rater;   %-nu.*Rv1;
