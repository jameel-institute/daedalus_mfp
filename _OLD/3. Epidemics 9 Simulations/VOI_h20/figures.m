...
diseases   = {'Influenza 2009','Influenza 1918',...
              'Covid Omicron','Covid Wildtype',...
              'SARS'};
strategies = {'No Closures','Economic Closures','No Closures','Economic Closures','Economic Closures'};
...
%     yt   = 100*T.SECt./gdp;
    ...
    ydat = nben;%%
    ylow = quantile(ydat,0.005);
    yupp = quantile(ydat,0.995);
    yran = linspace(ylow,yupp,1000);
    ...
    plot(xval-pdp,yran,'Linewidth',0.5,'Color','black');
    fill([xval+pdf,xval-fliplr(pdf)],[yran,fliplr(yran)],col,'FaceAlpha',0.5,'EdgeColor','none');
    plot(xval,mean(ydat(~isnan(ydat))),'Marker','s','MarkerSize',7.5,'Linewidth',1.5,'MarkerEdgeColor','black','MarkerFaceColor','white');
...
for i = 2;%:length(igroups);
 for   j = 1:4;%length(diseases);
    ...
    if strcmp(inp1,'HIC');
        start = 0;
        ylimmin    = [-300 -200 -500 -200 -300];
        ylimmax    = [0 200 2500 800 0];
        col = 'blue';
    else
    %gdp  = 365*sum(table2array(T(:,331:375)),2);  
    hb      = (0.20*T.Hmax*50*10^6/10^5);
    hbd     = hb*165;
    m       = 10*799.17;%mean and var of hbd in £
    v       = 535.37^2;
    mu      = log((m^2)/sqrt(v+m^2));
    sigma   = sqrt(log(v/(m^2)+1));
    r       = lognrnd(mu,sigma);%cost of hbd in million $
    start   = hbd*r/10^6;
    ylimmin    = [-150 -400 -200 -200 -400];
    ylimmax    = [100 1000 1000 1000 0];
    col = [1 0.5 0];
    end
    ...
    n        = 100;
    r        = 0.03;             
    EPV_cost = start+intcost.*sum(1./(1+r).^[0:n-1]);
    EPV_gain = p.*sben*sum(1./(1+r).^[0:n-1]);
    ROI      = 100*(EPV_gain-EPV_cost)./EPV_cost;
...
%     u = (d/2)*(1/1)+(1-1)*(1/1);
%     h = (1-d)*(1/1);
%     set(s,'position',[i u+0.05 w h]);
%     hold on;
    
    s = subplot(2,length(diseases),j+(i-1)*length(diseases));
    d = 0.2;
    in = 0.02+d*(0.975*1/5)+(j-1)*(0.975*1/5);
    w = (1-d)*(0.95*1/5);
    u = (d/2)*(1/2)+(2-i)*(1/2);
    h = (1-d-0.05)*(1/2);
    if i==1;
    set(s,'position',[in u+0.02 w h]);
    else
    set(s,'position',[in u+0.08 w h]);
    end
    hold on;

    %nl = min(sum(~isnan(ydat),1);
    
    b = boxchart(ydat, 'MarkerStyle', 'none','WhiskerLineColor','white','BoxFaceAlpha',0.25,'BoxFaceColor',col);
    for q = 1:length(p);
        vec = ydat(:,q);
        mdvec(q) = quantile(vec(~isnan(vec)),0.5);
        mnvec(q) = mean(vec(~isnan(vec)));
    end
    plot(mnvec,'ko','MarkerFaceColor', 'k');
    disp(round(min(mnvec)));
    disp(round(max(mnvec)));
    %plot(mdvec,'k-','MarkerFaceColor', 'k');
%     xval = 0.5;
%     col  = [1,1,0];%[0 0 1];%[1 .5 0]
%     plot(xval+pdp,yran,'Linewidth',0.5,'Color','black');
%     plot(xval-pdp,yran,'Linewidth',0.5,'Color','black');
...
%     plot(xval,mean(ydat(~isnan(ydat))),'Marker','s','MarkerSize',7.5,'Linewidth',1.5,'MarkerEdgeColor','black','MarkerFaceColor','white');
    ...
    if (j==1)&&(i==2);
        ...
        %ylabel('Societal Benefit (\% of GDP)');
        vec_pos = get(get(gca,'ylabel'),'Position');
        set(get(gca,'ylabel'),'Position',vec_pos + [-0.1 175 0]);
    end
    if (j==3)&&(i==2);
        xlabel('Annual Occurence Probability');
        %ylabel('Societal Benefit (\% of GDP)');
        vec_pos = get(get(gca,'xlabel'),'Position');
        set(get(gca,'xlabel'),'Position',vec_pos + [0 -200 0]);
    end
    set(gca,'FontSize',fs);  
