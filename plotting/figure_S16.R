...
gg <- ggplot(output_data, aes(x = strategy, y = value, fill = vlyl)) + #, alpha = min_any)) +
      ...
      facetted_pos_scales(y = list(
        ...
        scale_y_continuous(limits=c(0,8),   breaks=seq(0,8,2),     expand=c(0,0), position="right"),
...
output_table <- output_data %>%
                ...
                group_by(location, disease, strategy) %>%
                summarise(median_1 = quantile(vlyl_1, 0.50),
                          q1_1     = quantile(vlyl_1, 0.25),
                          q3_1     = quantile(vlyl_1, 0.75),
                          median_2 = quantile(vlyl_2, 0.50),
                          q1_2     = quantile(vlyl_2, 0.25),
                          q3_2     = quantile(vlyl_2, 0.75),
                          median_3 = quantile(vlyl_3, 0.50),
                          q1_3     = quantile(vlyl_3, 0.25),
                          q3_3     = quantile(vlyl_3, 0.75),
                          median_4 = quantile(vlyl_4, 0.50),
                          q1_4     = quantile(vlyl_4, 0.25),
                          q3_4     = quantile(vlyl_4, 0.75),
                ...
                mutate(median_4 = if_else(str_detect(strategy, "Elimination") & disease == "Covid-Wildtype-X",  paste0(median_4,"\\phantom{.}"), median_4)) %>%
