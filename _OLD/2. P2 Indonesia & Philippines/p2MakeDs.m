function [f,data]=p2MakeDs(data,NN,x,wfh)
...
NNrea = repmat(NN(1:lx)'/sum(NN(1:lx)),lx,1);%proportion of adult population in each sector as vector
...
HospInd = [32,43,44];%hospitality sector indices
...
if lx==45
    %Education:
    matA(lx+1,lx+1)=    matA(lx+1,lx+1)+    w(EdInd)^2*  data.schoolA1;%mixing within age groups only
    matA(lx+2,lx+2)=    matA(lx+2,lx+2)+    w(EdInd)^2*  data.schoolA2;
    
    %Hospitality:
    psub=data.NNs(HospInd);
    psub=sum(psub.*x(HospInd))/sum(psub);%constant from 0-1, weighted measure of how much sectors are open
...
%Transport:
matA(1:lx,1:lx)=    matA(1:lx,1:lx)+    repmat(w',lx,1).*   data.travelA3(1).*  NNrea.*  repmat(1-wfh,lx,1).*repmat(1-wfh',1,lx);%home-working has a compound effect
