...
output_data  <- lapply(output_files, add_scenario_cols) %>% bind_rows() %>% order_scenario_cols() %>%
                ...
                pivot_longer(cols = starts_with("fsiz_"), names_to = "fsiz", values_to = "value") %>%
                mutate(fsiz = factor(fsiz, levels = c("fsiz_1", "fsiz_2", "fsiz_3", "fsiz_4")))

gg <- ggplot(output_data, aes(x = strategy, y = value, fill = fsiz)) + 
      facet_grid2(disease ~ location, switch = "y", scales = "free_y") +
      geom_boxplot(outlier.shape = NA, coef = 1.5, width = 0.70, linewidth = 0.2) +
      scale_fill_manual(values = c("fsiz_1" = "magenta", "fsiz_2" = "green", "fsiz_3" = "brown", "fsiz_4" = "grey80"),
      ...
      scale_y_continuous(limits=c(0,100000), expand=c(0,0), position="right") +
      theme(panel.spacing = unit(0.75, "lines"), axis.text.x = element_text(angle = 55, hjust = 1)) + 
      labs(title = "", x = "", y = "Final Size (per 100k)") +
